...
config_root = 'config/'  # 토큰 파일이 저장될 폴더, 제3자가 찾지 어렵도록 경로 설정하시기 바랍니다.
...
_base_headers = {
    ...
    "Accept": "text/plain",
    "charset": "UTF-8",
    'User-Agent': AUTH_CONFIG['KIS_AGENT']
}
...
def changeTREnv(token_key, svr='prod', product=AUTH_CONFIG['PROD_DIGIT']):
    cfg = dict()

    global _isPaper
    if svr == 'prod':  # 실전투자
        ak1 = 'APP_KEY'  # 실전투자용 앱키
        ak2 = 'APP_SECRET'  # 실전투자용 앱시크리트
        _isPaper = False
    elif svr == 'vps':  # 모의투자
...
def auth(svr='prod', product=AUTH_CONFIG['PROD_DIGIT'], url=None):
    p = {
    ...
    if svr == 'prod':  # 실전투자
        ak1 = 'APP_KEY'  # 앱키 (실전투자용)
        ak2 = 'APP_SECRET'  # 앱시크리트 (실전투자용)
    elif svr == 'vps':  # 모의투자
        ak1 = 'paper_app'  # 앱키 (모의투자용)
        ak2 = 'paper_sec'  # 앱시크리트 (모의투자용)

    # 앱키, 앱시크리트 가져오기
    p["appkey"] = AUTH_CONFIG[ak1]
    p["appsecret"] = AUTH_CONFIG[ak2]

    # 기존 발급된 토큰이 있는지 확인
    saved_token = read_token()  # 기존 발급 토큰 확인
    # print("saved_token: ", saved_token)
    if saved_token is None:  # 기존 발급 토큰 확인이 안되면 발급처리
        base_url = API_CONFIG['BASE_URL'] if svr == 'prod' else API_CONFIG['DEV']['BASE_URL']
...
def reAuth(svr='prod', product=AUTH_CONFIG['PROD_DIGIT']):
    n2 = datetime.now()
